...
const server = setupServer(
  rest.post('http://localhost:7890/api/v1/shorten', (req, res, ctx) => {
    return res(ctx.json({
      id: 'abcd9876',
  ...
  rest.get('http://localhost:7890/api/v1/shorten', (req, res, ctx) => {
    return res(ctx.json([
      { id: 'abcd1234', originalUrl: 'http://test.com/long/url/0' },
      { id: 'abcd1235', originalUrl: 'http://test.com/long/url/1' },
      { id: 'abcd1236', originalUrl: 'http://test.com/long/url/2' }
    ]));
  })
);

describe('Shorten contanier', () => {
  ...
  it('creates a short url with form submission', async() => {
    ...
    return waitFor(() => {
      screen.getByText('http://example.com/long/url', { exact: false });
    });
  });
